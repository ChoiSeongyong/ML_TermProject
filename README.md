...
### 2. 클래스 불균형 해결: 각 클래스의 빈도 역수(Inverse-Frequency)를 기반으로 계산된 `weighted CrossEntropyLoss`를 적용하여 소수 클래스에 대한 학습력 강화 
### 3. 하이퍼파라미터 튜닝: ResNet-50과 Swin-T 각각에 대해 **Validation Macro F1-Score**를 기준으로 Learning Rate Grid Search를 수행 (5 Epochs)한 뒤, 최적의 LR로 본 학습(20 Epochs) 진행
### 4. 다각적 성능 평가: Accuracy, Macro/Weighted F1-Score, ROC-AUC (One-vs-Rest) 및 오분류 쌍 히트맵(Misclassification Pairs Heatmap) 분석
